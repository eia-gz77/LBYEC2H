...
\begin{enumerate}
    \item START
    \item DEFINE symbolic variable `t'
    \item CONSTRUCT signal x(t) using Heaviside functions for interval [0, 8]
    \item FOR EACH transformation (shift, flip, scale):
    \begin{itemize}
        \item APPLY transformation to t
        \item PLOT result in 3x2 subplot grid (Figure 1)
    \end{itemize}
    \item CONSTRUCT signal y(t) = exp(-t)u(t) + u(-1-t)
    \item PLOT y(t) for t in range [-6, 6] (Figure 2)
    \item DEFINE h1(t) through h4(t) for LPF, HPF, BPF, and BSF
    \item OVERLAY h1(t) and h2(t) (excluding delta) on same axis (Figure 3)
    \item CONVERT symbolic x(t) to numeric array x\_num with dt = 0.01
    \item COMPUTE Rxx = xcorr(x\_num, x\_num) * dt
    \item PLOT Rxx vs tau (Figure 4)
    \item END
\end{enumerate}